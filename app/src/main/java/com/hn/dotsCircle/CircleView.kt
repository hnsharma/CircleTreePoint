...
class CircleView :
    ...
    fun findCircle(
        ...
        val h = -g
        val k = -f
        val sqr_of_r = Math.pow((h - x1),2.0) + Math.pow((k - y1),2.0)
        ...
        var paintPont  = Paint()
        paintPont.color  = Color.RED
        paintPont.strokeWidth = 10f


        var paintPont1  = Paint()
        paintPont1.color  = Color.RED
        paintPont1.strokeWidth = 10f
        paintPont1.setStyle(Paint.Style.STROKE);
        canvas.drawCircle(h.toFloat(),k.toFloat(),r.toFloat(),paintPont1)
    }
